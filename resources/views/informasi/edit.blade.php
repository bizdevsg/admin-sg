...
@section('main-content')
    <h1 class="h3 font-weight-bold mb-3">Edit Informasi</h1>
        ...
        <div class="card-body">
            <form action="{{ route('informasi.update', $informasi->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $informasi->title) }}" required>
                    @error('title')
                ...
                <div class="form-group">
                    ...
                    <textarea name="content" id="content" rows="6" class="form-control tinymce-editor @error('content') is-invalid @enderror" required>{{ old('content', $informasi->content) }}</textarea>
                    @error('content')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    @if ($informasi->image)
                        <div class="mb-2">
                            <img src="{{ asset($informasi->image) }}" alt="{{ $informasi->title }}" style="max-height: 120px;">
                        </div>
                    @endif
